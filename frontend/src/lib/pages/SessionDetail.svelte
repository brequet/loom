<script lang="ts">
    ...
    $effect(() => {
        ...
        if (
            session &&
            (session.state === "provisioning" || session.state === "running")
        ) {
    ...
    let fields = $derived<DetailField[]>(
        session
            ? [
                ...
                <div class="space-y-1 min-w-0">
                    ...
                    >
                        <Badge variant="secondary" class="capitalize text-xs"
                            >{session.source_type}</Badge
                        >
                        {#if session.source_ref}
                            <span class="font-mono text-xs"
                                >{session.source_ref}</span
                            >
                        {/if}
                    </div>
                </div>
        ...
        <div class="space-y-1">
            {#each fields as field}
                <div class="flex items-baseline py-2 gap-4">
                    ...
                    >
                    <span
                        class="text-sm {field.mono ? 'font-mono' : ''} truncate"
                    >
                        {field.value ?? "—"}
                    </span>
                </div>
